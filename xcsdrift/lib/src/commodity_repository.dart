...
class CommodityRepository implements RepositoryBase {
  ...
  Future<List<BiFacetBi>> loadCommodities({String tenantId = 'default'}) async {
    var rs = await portalManager.loadAsBiFacetsByTenant(
        bundleName: _bundleName, tenantId: tenantId);
    return rs;
  }

  Future<List<ent.Commodity>> readFromFile(File file) async {
    final String fileCnt = await file.readAsString();
    final rs = json.decode(fileCnt) as List;
    List<ent.Commodity> facs = ent.asCommodities(rs);
    return facs;
  }
  ...
  Future<void> storeEntry(Map<String, dynamic>? jsonEl, {Batch? batch}) async {
    ...
    _logger.info("insert ${dataMap['commodity_id']}");
    var entry = CommodityData.fromJson(dataMap);
    if (batch == null) {
      await database.batch((batch) {
        batch.insert(database.commodity, entry,
  ...
  Future<ent.Commodity> fetchSingle(String bundleId) async {
    var jsonEl=await facetStorage.get(fullBundleName: _fullBundleName, key: bundleId);
    final elData = ent.Commodity.fromJson(jsonEl);
  ...
  Future<ent.Commodity?> smartFetchSingle(String bundleId,
  ...
  Future<ent.Commodity?> checkRefresh(
      ent.Commodity remoteData, String bundleId) async {
  ...
  Future<List<ent.Commodity>> fetchMulti(List<String> ids, {bool smartMode=false}) async {
  ...
  Future<List<ent.Commodity>> fetchFromReg(String regNode, {bool smartMode=false}) async {
  ...
  Future<List<ent.Commodity>> fetchFromSrv({String tenantId = 'default', bool smartMode=false}) async {
    List<BiFacetBi> elements = await loadCommodities(tenantId: tenantId);
    return await storeEntries(elements, smartMode: smartMode);
  }
  ...
  Future<List<ent.Commodity>> storeEntries(List<BiFacetBi> elements, {bool smartMode=false}) async {
    var rs=<ent.Commodity>[];
    await database.batch((batch) {
      for (var el in elements) {
        // 不加"fromJson->toJson"的转换会导致drift无法处理list元素的cast.
        final elData=ent.Commodity.fromJson(el.data!);
        rs.add(elData);
  ...
  Future<List<ent.Commodity>> storeDs(List<Map<String, dynamic>> ds, {bool smartMode=false}) async{
    var rs=<ent.Commodity>[];
    await database.batch((batch) {
      for (var el in ds) {
        final elData=ent.Commodity.fromJson(el);
        rs.add(elData);
        writeLocal(elData, batch, smartMode: smartMode);
      }
    });
  ...
  Future<void> writeLocal(ent.Commodity elData, Batch batch, {bool smartMode=false}) async{
    if(smartMode) {
      await checkRefresh(elData, elData.commodityId!);
    }else {
  ...
  Future<void> storeEnts(List<ent.Commodity> elements) async{
    await database.batch((batch) {
      for (var el in elements) {
  ...
  Future<List<ent.Commodity>> fetchFromLocalFile(File file) async {
    List<ent.Commodity> ds = await readFromFile(file);
    await database.batch((batch) {
      for (var el in ds) {
  ...
  CommodityDrift get tbl => database.commodityDrift;

  Future<String> add(ent.Commodity rec) async {
  ...
  Future<ent.Commodity?> getAsEnt(String id) async {
    var rec = await get(id);
    return convRecord(rec);
  }

  ent.Commodity? convRecord(CommodityData? rec) {
    if (rec != null) {
      Map<String, dynamic> normMap = normalizeMap(rec);
      return ent.Commodity.fromJson(normMap);
    }
    return null;
  }

  Future<int> remove(String id) async {
    return await tbl.deleteCommodity(id: id);
  }

  Future<List<CommodityData>> all() async {
    return await tbl.allCommodities().get();
  }

  Stream<List<CommodityData>> watchAll() {
    return tbl.allCommodities().watch();
  }

  Stream<CommodityData> watchSingle(String id){
    return tbl.getCommodity(id).watchSingle();
  }
  ...
  Future<int> touch(String id) async {
    var sett = database.update(database.commodity)
      ..where((el) => el.commodityId.equals(id));
    var result = await sett
        .write(CommodityCompanion(lastUpdatedTxStamp: Value(DateTime.now())));
    return result;
  }
