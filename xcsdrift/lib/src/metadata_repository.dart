...
class MetadataRepository extends RepositoryBase {
  ...
  Future<int> setBy(Map<String, String> cond, MetadataCompanion values) async {
    var filter = database.buildQueryExprs(cond);
    var sett = database.update(database.metadata)..where((el) => filter);
    values = values.copyWith(lastUpdatedTxStamp: Value(DateTime.now()));
    return await sett.write(values);
  }

  /// Get records by condition-map
  Future<List<MetadataData>> getBy(Map<String, String> cond) async{
    var q=db.select(db.metadata)..where((el)=>database.buildQueryExprs(cond));
    return await q.get();
  }
...
extension MetadataPagedEx on PaginatedResponse{
  List<ent.Metadata> asMetadata(){
    var rs = results?.map((el) => ent.Metadata.fromJson(el)).toList();
    return rs ?? <ent.Metadata>[];
  }
}
...
extension MetadataQueryEx on Database {
  SimpleSelectStatement<Metadata, MetadataData> queryMetadata(Map<String, String> exprs) {
    var filter = buildQueryExprs(exprs);
    return select(metadata)..where((u) => filter);
  }

  SimpleSelectStatement<Metadata, MetadataData> paginatedMetadata(
      Map<String, String> exprs, int pageIndex, {int pageSize=5}) {
    var filter = buildQueryExprs(exprs);
    var start = pageIndex * pageSize;
    _logger.info('.. offset $start, limit $pageSize');
    return select(metadata)..where((u) => filter)..limit(pageSize, offset: start);
  }
}
