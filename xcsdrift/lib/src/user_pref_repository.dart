...
class UserPrefRepository implements RepositoryBase {
  ...
  Future<List<ent.UserPref>> fetchMulti(List<String> ids) async {
    final elements=await facetStorage.multiGet(fullBundleName: _fullBundleName, keys: ids);
    return await storeDs(elements);
  }

  Future<List<ent.UserPref>> fetchFromReg(String regNode) async {
  ...
  Future<List<ent.UserPref>> fetchFromSrv({String tenantId = 'default'}) async {
  ...
  Future<void> store(ent.UserPref data) async {
    await storeEntry(data.toJson());
  }
  Future<void> storeAndPush(ent.UserPref data) async {
    await store(data);
    await push(data);
  }
  

  Future<List<ent.UserPref>> storeEntries(List<BiFacetBi> elements) async {
    var rs=<ent.UserPref>[];
    await database.batch((batch) {
  ...
  Future<List<ent.UserPref>> storeDs(List<Map<String, dynamic>> ds) async{
    var rs=<ent.UserPref>[];
    await database.batch((batch) {
      for (var el in ds) {
